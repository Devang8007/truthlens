...
import { FileText, Loader2, AlertCircle, Sparkles, Globe, AlignLeft } from 'lucide-react';
import api from '../../api';
  ...
  return (
    <div className="max-w-4xl mx-auto py-3 sm:py-6">
      <div className="mb-6 sm:mb-8 animate-in fade-in slide-in-from-top-4">
        <div className="inline-flex items-center space-x-2 bg-blue-50 text-blue-600 px-3 py-1 rounded-full text-xs font-bold mb-2.5 sm:mb-3">
          <Sparkles className="h-3.5 w-3.5 shrink-0" />
          <span className="truncate">RoBERTa Misinformation Classifier & Fact-Check Cross-Reference</span>
        </div>
        <h1 className="text-2xl sm:text-3xl font-black text-dark tracking-tight mb-2">Fake News & Misinformation Verification</h1>
        <p className="text-xs sm:text-sm text-gray-500">Scan news articles, social media posts, or headline claims for bias, factual inaccuracies, and coordinated disinformation.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6 mb-8">
        <div className="lg:col-span-2">
          <div className="glass-card p-4 sm:p-6">
            {/* Input Toggle */}
            <div className="flex flex-col xs:flex-row gap-1.5 sm:gap-2 mb-5 sm:mb-6 p-1 bg-gray-100/80 rounded-xl">
              <button
                type="button"
                onClick={() => { setInputType('text'); setContent(''); setError(''); }}
                className={`flex-1 py-2 sm:py-2.5 px-3 sm:px-4 rounded-lg font-bold text-xs flex justify-center items-center space-x-2 transition-all cursor-pointer ${
                  inputType === 'text'
            ...
            <div className="mb-4">
              {inputType === 'text' ? (
                <div>
                  ...
                  />
                  <div className="flex flex-col xs:flex-row justify-between items-start xs:items-center gap-1 mt-2 px-1 text-[11px] sm:text-xs text-gray-400">
                ...
                <div>
                  <input
                    ...
                    onChange={(e) => setContent(e.target.value)}
                  />
                  <p className="text-[11px] sm:text-xs text-gray-400 mt-2 px-1">
            ...
            {loading && (
              <div className="mt-4 mb-6 p-4 rounded-xl bg-blue-50 border border-blue-100 animate-in fade-in">
                <div className="flex items-center space-x-3 mb-2">
                  ...
                  <p className="text-xs sm:text-sm font-bold text-blue-900">{scanSteps[scanStep]}</p>
                </div>
            ...
            <div className="flex justify-end pt-2">
              <button
                ...
                className="btn-primary w-full sm:w-auto space-x-2 bg-blue-600 hover:bg-blue-700 shadow-blue-500/20 justify-center"
              >
                {loading ? (
                  <>
        ...
        <div className="space-y-4 sm:space-y-6">
          <div className="glass-card p-5 sm:p-6">
            <h3 className="text-xs sm:text-sm font-bold text-dark uppercase tracking-wider mb-4">NLP Evaluation Signals</h3>
